...
class TestCollada2Eus(unittest.TestCase):
    def check_euscollada(self,filename,function):
        ...
        callstr = 'rosrun euslisp irteusgl \"(defun my-exit (&rest args) (unix::_exit -1))\" \"(lisp::install-error-handler #\'my-exit)\" \"(load \\"test/'+filename+'.l\\")\" \"(when (and x::*display* (> x::*display* 0))(objects ('+function+')) (send *viewer* :draw-objects) (send *viewer* :viewsurface :write-to-image-file \\"test/'+filename+'.ppm\\"))\" \"(exit 0)\"'
